...
# Combined Intake Family, Income, and Goals Design QA

## Comparison target

- Source visual truth: `C:\Users\amans\AppData\Local\Temp\codex-clipboard-4d8eea21-8b73-4ae1-81e3-de9eee0e17b8.png` and the attached Family build specification images.
- Browser-rendered Family frame: `C:\Users\amans\.codex\visualizations\2026\08\15\01a006a6-49e3-7201-acc2-0cc51a072b8f\family-implementation-clean-1280x498.png`.
- Combined same-state comparison: `C:\Users\amans\.codex\visualizations\2026\08\15\01a006a6-49e3-7201-acc2-0cc51a072b8f\family-reference-comparison.png`.
- Responsive evidence: `verify-out/wizard-family-mobile.png`, `verify-out/wizard-income-mobile.png`, and `verify-out/wizard-goals-mobile.png`.

## Comparison evidence

- The reference and implementation were reviewed together at the specification's 1280px design width using the same two-person state, dates of birth, employment statuses, retirement ages, longevity ages, and collapsed Children state.
- The production implementation matches the reference rail width, header and footer heights, content origin, two compact person cards, card gap, field order, warm hairlines, restrained radii, dark surfaces, gold focus state, and typography hierarchy.
- The six-step rail is intentionally authoritative: Family, Net Worth, Income, Goals, Tax, Summary. Filing status and residence intentionally moved from Family into Tax, so Children sits directly below the person cards.
- The existing production household title and top-right status shell remain inherited behavior; this candidate adds no browser-local household save model or new saved/unsaved language.
- Income and Goals use the same production design system. Income exposes canonical Social Security, recurring income, pension, and savings inputs; Goals embeds the existing canonical Goals Horizon instead of cloning its state or behavior.

## Findings and iteration

- The Income screen is frozen at the owner's direction. This candidate makes no
  later discretionary Income redesign; any future Income or Tax visual redesign
  is outside this Intake candidate.
- Initial 390px QA found Family Status, Retires at, and Live to age controls extending to x=566 outside the x=390 viewport. The mobile grid now keeps every milestone control within x=340 and the verifier asserts those bounds.
- Initial 390px QA found the 720px Goals Horizon clipped inside a 337px mount with `overflow-x: visible`. The mount now owns horizontal scrolling with `overflow-x: auto`, preserving the canonical timeline without widening the document.
- Summary now represents a canonical `startsAtRetirement` goal as `retirement` instead of an unavailable dash.
- The design-width Family comparison intentionally retains the production household shell and omits the reference's former filing/residence row because those ownership decisions are part of the approved combined scope.
- The owner-directed IRMAA filing-status width correction fixes the track at 240px
  on wide layouts and 220px at the narrower breakpoint instead of filling the row.

## Verification

- Focused syntax and source-contract checks passed for engine Income authority,
  planning catalog/tax treatment, exact savings allocations, co-client removal,
  canonical Goals, Tax separation, Summary consumption, and Family DOB authority.
- `npm test`: pretest 24 passed, 0 failed; main suite 706 passed, 0 failed.
- `git diff --check`: passed (line-ending conversion warnings only).
- `npm run verify`: passed. The live production browser contract covered all six
  Intake steps, co-client removal, Income/Tax source separation, Goals reuse,
  reload behavior, responsive bounds, and screenshots at the canonical origin.
